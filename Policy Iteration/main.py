...
n = int(input("Enter the size of the Tic-Tac-Toe : "))
...
# policy = []
# for i in range(3139):
#     policy.append(random.randint(0,8))

def get_1step_reward_function(policy):
    R_ = np.zeros(shape=(3139,0))
    for i in range(len(state_space)):
        for j in range(len(state_space)):
            R_[i]+=prob_tans_func[i][policy[i]][j]
    return R_


def get_value_function(W, R_, policy):
    P = np.zeros(shape=(3139,3139))
    for i in range(len(state_space)):
        for j in range(len(state_space)):
            P[i][j] = gamma*prob_tans_func[i][policy[i]][j]

    I = np.zeros(shape=(3139,3139))
    for i in range(len(state_space)):
        I[i][i] = 1

    M = np.zeros(shape=(3139,3139))
    for i in range(len(state_space)):
        for j in range(len(state_space)):
            M[i][j] = I[i][j]-P[i][j] #
    I_YP_1 = np.linalg.inv(M)
    W1 = np.matmul(I_YP_1,R_)
    W = W.copy()
    return W

def policy_extraction(W,policy):
    for i in range(len(state_space)):
        temp_vals = []
        for a in range(len(action_space)):
            temp = 0
            for j in range(len(state_space)):
                temp+=prob_tans_func[i][a][j]*(reward_function[i][a][j]+gamma*W[j])
            temp_vals.append(temp)
        maxi = -400
        act = -1
        for a in range(len(temp_vals)):
            if(maxi<temp_vals[a]):
                policy[i] = a
    return policy

policy = []
W = np.zeros(shape=(3139,1))
for iter in range(1000): #

    for i in range(3139):
        policy.append(random.randint(0,8))
    R_ = get_1step_reward_function(policy)
    Wnew = get_value_function(W, R_, policy)
    policy_new = policy_extraction(Wnew, policy)
    W = Wnew
    policy = policy_new
